...
class StepperMotorDriver:
    ...
    def reverse_direction(self):
        ...
        # self.set_dir(direction=self.direction)
        self.set_frequency(freq=-self.frequency)
    ...
    def rotate(
        ...
        if not self.pwd_chan:
            self.enable_driver()
            self.pwd_chan = GPIO.PWM(self.step_pin, abs(self.frequency))  # pin, freq
            self.pwd_chan.start(duty_cycle)
        else:
            self.pwd_chan.ChangeFrequency(abs(self.frequency))
